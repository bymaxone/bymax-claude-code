...
| **OpenAI Codex plugin** — `codex@openai-codex` | `bymax-quality` — Review C, the adversarial second opinion, only under `/bymax-quality:code-review --adversarial` | `claude plugin marketplace add openai/codex-plugin-cc`, then `claude plugin install codex@openai-codex` — plugin `codex`, marketplace `openai-codex`, repo `codex-plugin-cc`, all three needed as written |
