...
| --- | --- |
| plugin not found among installed, enabled plugins — **this one line covers both absent and merely disabled**, since the runtime selects on `.enabled == true` | if it is installed but off, `claude plugin enable codex@openai-codex`; otherwise install it — `claude plugin marketplace add openai/codex-plugin-cc`, then `claude plugin install codex@openai-codex` |
...
| version `X` is not a verified version | the script's contract with that runtime is undocumented and was read in the listed versions only — the message names them, and `COMPANION_VERIFIED_VERSIONS` in `scripts/codex-review.sh` is the source. Installing a listed version is not an option: `claude plugin install` takes no version. So either wait for the plugin to be re-verified, or run the unverified one anyway with `BYMAX_CODEX_COMPANION_ALLOW_UNVERIFIED=1` — the user's explicit decision. Neither route restores the **pinned** path: `limits_known` is set only by a match against `COMPANION_VERIFIED_VERSIONS`, so a runtime admitted any other way never answers `ok` — its successful runs answer `ok-unpinned`. That holds for the `BYMAX_CODEX_COMPANION` override in the rows below too, whose version the script reports as the literal `override` |
