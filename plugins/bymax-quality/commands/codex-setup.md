...
| --- | --- |
| plugin not found among installed, enabled plugins — **this one line covers absent, merely disabled, and a listing that would not parse**, since the runtime selects on `.enabled == true` and treats an unreadable listing as no match | if it is installed but off, `claude plugin enable codex@openai-codex`; otherwise install it — `claude plugin marketplace add openai/codex-plugin-cc`, then `claude plugin install codex@openai-codex` |
